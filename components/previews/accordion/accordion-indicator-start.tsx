...
// The indicator sits at the start of the header instead of the end.
export function AccordionIndicatorStartExample() {
  return (
    <Accordion defaultValue={["item-1"]} className="w-full max-w-md">
      {faqs.map((faq) => (
        <AccordionItem key={faq.value} value={faq.value}>
          <AccordionTrigger indicatorPosition="start">{faq.question}</AccordionTrigger>
